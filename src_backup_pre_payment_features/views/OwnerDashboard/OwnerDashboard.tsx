...
import { Users, Award, Heart, Printer, ArrowRight, Clock, LogOut, Search } from 'lucide-react';
...
export const OwnerDashboard: React.FC = () => {
  ...
  const t = {
    ...
    posterIssuer: language === 'ko' ? '발행기관: 로컬 상점 기부 로열티 플랫폼 ShareStamp' : 'Issued by: Local Store Donation Loyalty Platform ShareStamp',
  ...
  const handleOwnerLogin = (e: React.FormEvent) => {
    ...
    const user = users.find(u => u.role === 'owner' && u.loginId === loginIdInput);
  ...
  const selectedStore = (currentOwner && (stores.find(s => s.id === selectedStoreId && s.ownerId === currentOwner.id) || stores.find(s => s.ownerId === currentOwner.id))) || stores[0] || { id: 'store_id_1', name: 'ShareStamp 매장', category: 'cafe', pointRewardPer7Stamps: 5, currency: 'USD', earningIntervalMinutes: 60, ownerId: 'none' };
  ...
  const getHistoricalMockData = (index: number, type: 'daily' | 'weekly' | 'monthly') => {
    const mult = selectedStore.pointRewardPer7Stamps;
    const charSum = selectedStoreId.split('').reduce((sum, char) => sum + char.charCodeAt(0), 0);
    const seed = (charSum + index * 17) % 100;
    
    let stampsIssued = 0;
    let stampsDonated = 0;
    
    if (type === 'daily') {
      stampsIssued = 15 + (seed % 15);
      stampsDonated = 2 + (seed % 5);
    } else if (type === 'weekly') {
      stampsIssued = 100 + (seed % 80);
      stampsDonated = 12 + (seed % 25);
    } else {
      stampsIssued = 450 + (seed % 250);
      stampsDonated = 50 + (seed % 90);
    }
    
    const conversionRate = 0.12 + (seed % 5) / 100;
    const cashConverted = Math.round(stampsIssued * conversionRate * mult * 10) / 10;
    const posDiscountApplied = Math.round(cashConverted * (0.8 + (seed % 3) / 10) * 10) / 10;
    const donationCashValue = Math.round(stampsDonated * (mult / 7) * 100) / 100;
  ...
  const getAggregatedStats = (type: 'daily' | 'weekly' | 'monthly') => {
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const mockStats = getHistoricalMockData(i, type);
      
      results.push({
        period: label,
        stampsIssued: realStats.stampsIssued + mockStats.stampsIssued,
        cashConverted: realStats.cashConverted + mockStats.cashConverted,
        posDiscountApplied: realStats.posDiscountApplied + mockStats.posDiscountApplied,
        stampsDonated: realStats.stampsDonated + mockStats.stampsDonated,
        donationCashValue: realStats.donationCashValue + mockStats.donationCashValue
      });
    }
    return results;
  };
  ...
  const handleExportExcel = () => {
    ...
    setTimeout(() => {
      ...
      setToastMessage(t.exportExcelSuccess(`SHARESTAMP_Settlement_${selectedStoreId}_${todayStr}.xlsx`));
    }, 1200);
  };
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <h2 style={{ fontSize: '24px', fontWeight: 900, color: 'var(--primary-color)' }}>ShareStamp</h2>
                ...
                <>
                  ...
                  {signUpStoreSearchQuery.trim().length > 0 && (() => {
                    ...
                    const matches = stores.filter(s => {
                      ...
                      return isMatch && (!s.ownerId || s.ownerId === 'none' || s.ownerId === '');
                    });
    ...
    return (
      <div style={{ backgroundColor: '#ffffff', minHeight: '100vh', padding: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{
          ...
          <div>
            <h1 style={{ fontSize: '42px', fontWeight: 900, color: 'var(--primary-color)', marginTop: '20px', letterSpacing: '-1px' }}>ShareStamp</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
        ...
        <div className="imin-card" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: 'auto' }}>
            ...
            >
              {Array.from({ length: 20 }, (_, i) => (i + 1) * 30).map((mins) => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '20px' }}>
            ...
            <div className="imin-card" style={{ display: 'flex', alignItems: 'center', gap: '20px', backgroundColor: '#ffffff' }}>
              ...
              <div>
                ...
                <h3 style={{ fontSize: '24px', fontWeight: 800, marginTop: '4px', color: 'var(--accent-red)' }}>
                  ${(donations.filter(d => d.storeId === selectedStoreId && isSameMonth(d.createdAt, 0)).reduce((sum, d) => sum + d.monetaryValue, 0) + (45 * selectedStore.pointRewardPer7Stamps / 7)).toFixed(2)}
                  <span style={{ fontSize: '14px', fontWeight: 500, color: 'var(--text-secondary)', marginLeft: '6px' }}>
                    ({t.historyQty(donations.filter(d => d.storeId === selectedStoreId && isSameMonth(d.createdAt, 0)).reduce((sum, d) => sum + d.stampCount, 0) + 45)})
                  </span>
                </h3>
