...
// 옛 CustomerPWA 구성 차용: 매장별 7칸 허니컴 스탬프 카드 + 3출구(적립 전환·친구 선물·NPO 기부) + 나눔 임팩트.
// 로그인/친구선물은 다음 단계(로그인 포팅) — 지금은 deviceId 기반, 선물은 준비중.
...
function Hex({ filled }: { filled: boolean }) {
  return (
    <div
      className={`flex aspect-square items-center justify-center text-[13px] ${filled ? 'bg-brand-600 text-white' : 'bg-zinc-100 text-zinc-300'}`}
...
export default function MePage() {
  ...
  const [sheet, setSheet] = useState<{ card: Card; kind: 'donate' } | null>(null);
  ...
  const stampValue = (card: Card) => (card.reward / 7) * Math.min(card.currentStamps, 7);
    ...
    <main className="mx-auto max-w-xl px-4 pb-24">
            ...
            return (
              <section key={card.storeId} className="ss-card p-5">
                ...
                <div className="mt-1 text-xs text-zinc-500">
                  7개당 보상 <strong className="text-rose-500">${card.reward.toFixed(2)}</strong> · 현재 가치 <strong className="text-brand-700">${val.toFixed(2)}</strong>
                </div>
                <div className="mt-3 grid grid-cols-7 gap-1.5">
                  {Array.from({ length: 7 }).map((_, i) => (
                    <Hex key={i} filled={i < filled} />
                  ))}
                </div>
                <div className="mt-4 grid grid-cols-3 gap-2">
                  ...
                  <button onClick={() => { setSheet({ card, kind: 'donate' }); }} disabled={busy || !canUse} className="ss-chip justify-center py-2.5 text-sm disabled:opacity-40">기부 💛</button>
                </div>
      ...
      {donations.length > 0 && (
        <section className="ss-card mt-4 p-5">
          <h2 className="text-base font-extrabold">나눔 기부 내역 💛</h2>
      ...
      {sheet && (
        <div className="fixed inset-0 z-50 flex items-end bg-black/40" onClick={() => setSheet(null)}>
          <div className="w-full rounded-t-3xl bg-white p-5" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mt-1 text-sm text-zinc-500">{sheet.card.storeName}의 스탬프 가치 <strong className="text-brand-700">${stampValue(sheet.card).toFixed(2)}</strong>를 기부해요.</p>
